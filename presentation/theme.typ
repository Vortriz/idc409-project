...
#let inf-rules(
  ...
  formulas.pos()
    .map(f => if f == () { lb } else { box(inf-style(proof-tree(f))) })
    .join(" ")
  lb
}))
  ...
  let header(self) = {
    ...
    grid(
      ...
      if self.store.progress-bar {
        components.progress-bar(height: 3pt, self.colors.primary, self.colors.tertiary)
      },
      block(
        inset: (x: .5em),
    ...
    align(
      center + horizon,
      {
        block(
          ...
          breakable: false,
          {
            text(size: 1.8em, fill: self.colors.primary, strong(info.title))
        ...
        v(1em)
        line(length: 100%, stroke: 3pt + self.colors.primary.lighten(70%))
        set text(size: 1.2em)
        v(0.5em)
        grid(
          ...
          column-gutter: 1em,
          row-gutter: 1em,
          ..info.authors.map(author => text(fill: self.colors.primary.darken(25%), author))
        )
        v(1.5em)
        set text(size: 0.70em, fill: self.colors.tertiary.darken(50%).desaturate(50%))
        set align(left)
        text(extra)
        v(-0.2em)
        if info.institution != none {
          parbreak()
          text(size: .75em, info.institution)
        }
        if info.date != none {
          parbreak()
          v(-0.5em)
          text(size: .75em, utils.display-info-date(self))
        }
      },
    )
  }
  ...
  let slide-body = {
    ...
    custom-outline(
      ...
      transform: (hd, it) => {
        ...
        //  if ((hd.relation != none) and hd.relation.child) {
        //     set text(fill: self.colors.primary, style: "italic")
        //  }
        set text(size: 1.00em,fill: self.colors.primary, style: "italic") if hd.relation != none and hd.relation.child

        set text(size: 0.75em, fill: text.fill.transparentize(75%)) if hd.relation != none and hd.relation.sibling

        if it.element.body != [Bibliography] {
          it
        }
      }
    )
...
#let university-theme(
  ...
  show: touying-slides.with(
    config-page(
      ...
      header-ascent: 0em,
      footer-descent: 0em,
      margin: (top: 3em, bottom: 1.25em, x: 3em),
    ),
    config-common(
      slide-fn: slide,
